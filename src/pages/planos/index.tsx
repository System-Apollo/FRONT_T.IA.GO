export default function Planos () {
    ...
    return (
        <section className='py-14 bg-gradient w-full'>
            <div className="max-w-screen-xl mx-auto px-4 text-gray-600 md:px-8">
                <div className='relative max-w-xl mx-auto sm:text-center'>
                    ...
                    <div className='mt-3 max-w-xl text-gray-400'>
                        <p>
                            Escolha o plano ideal e o seu investimento em Tecnologia Jurídica
                        </p>
                    </div>
                </div>
                <div className='mt-16 justify-center gap-6 sm:grid sm:grid-cols-2 sm:space-y-0 lg:grid-cols-3'>
                    {
                        plans.map((item, idx) => (
                            <div key={idx} className={`relative flex-1 flex items-stretch flex-col rounded-xl border border-gray-700 bg-dark mt-6 sm:mt-0 ${item.isMostPop ? "mt-10" : ""}`}>
                                {
                                    item.isMostPop ? (
                                        <span className="w-32 absolute -top-5 left-0 right-0 mx-auto px-3 py-2 rounded-full border border-gray-700 shadow-md bg-gray-700 text-center text-indigo-50 text-sm font-semibold">Mais popular</span>
                                    ) : ""
                                }
